...
A self-hosted Telegram bot that downloads media via `yt-dlp`/`gallery-dl` and uploads it back to the user. Pure utility — no LLM calls. Built on **aiogram 3.x** (async). Targets homelab/Docker deployment.
...
In Docker, `docker-entrypoint.sh` refreshes yt-dlp and gallery-dl (`pip install -U`, best-effort) on container start unless `YTDLP_AUTO_UPDATE=false`, then launches the bot — so site breakages are fixed without an image rebuild.
